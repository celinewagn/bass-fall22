...
author: "Celine Wang" & "Zay Yar Han"
date: "September 2022"
output:
  html_document:
    df_print: paged
...
#anna's sheet:
urlfile="https://raw.githubusercontent.com/annamacfarlane/mice-behavioral/main/qial_animals_with_age2.csv"

anna<-read.csv(url(urlfile))
combo <- left_join(df, anna, by=c("ID" = "BadeaID"))
write.csv(anna, "/Users/celwa/Desktop/Fall 2022/BASS CONNECTIONS/NOR_Final/annainfo.csv", row.names = TRUE)

write.csv(combo, "/Users/celwa/Desktop/Fall 2022/BASS CONNECTIONS/NOR_Final/annacomb.csv", row.names = TRUE)
...
dfBase<-rbind(df2, df3, df4) #Base APOE subset
...
dfOMCHN3 <- subset(dfOMCHN, APOE == "E3HN")
dfYFHHN2 <- subset(dfYFHHN, APOE == "E2HN")

dfOMHHN3 <- subset (dfOMHHN, APOE == "E3HN")
dfYFHHN4 <- subset (dfYFHHN, APOE == "E4HN")
...
library(effectsize)
eta=eta_squared(oB)
etaB<- as.data.frame(eta_squared(oB))
cohensB<- as.data.frame(cohens_f(oB))
# HNadjusted <- as.data.frame(p.adjust(oHN_anova$`Pr(>F)`, method = "fdr"))

# total<- cbind(oHN_anova, eta)
#p.adjust cohens f is for ANOVA, and then hedges_g (cohen's d prob for mine) tukey_hsd(res.aov)

write.csv(cohensB, "/Users/celwa/Desktop/Fall 2022/BASS CONNECTIONS/Base_analysis/B_cohens.csv" )
write.csv(etaB, "/Users/celwa/Desktop/Fall 2022/BASS CONNECTIONS/Base_analysis/B_anova_eff.csv" )
write.csv(oB_anova, "/Users/celwa/Desktop/Fall 2022/BASS CONNECTIONS/Base_analysis/B_anova.csv")
#write.csv(CI, "/Users/celwa/Desktop/Fall 2022/BASS CONNECTIONS/HN_analysis/HN_CI.csv")
#write.csv(HNadjusted, "/Users/celwa/Desktop/Fall 2022/BASS CONNECTIONS/HN_analysis/HN_adj_pval.csv")
summary(oB)
```      
```{r}
#Base post-hoc analysis
posthoc1hn <- emmeans(oHN, list(pairwise~APOE*Sex*Age_Cat*Diet), adjust="tukey")
posthoc1hn[2]
pairwises_allHN <- ((as.data.frame(posthoc1hn[2]))) # $'pairwise differences of APOE, sex, age, diet'
fdr_adjusted_pvals <- p.adjust((pairwises_allHN[ , "pairwise.differences.of.APOE..Sex..Age_Cat..Diet.p.value"]), method = "fdr") 
...
```
```{r}
#COMPARING AGES FOR BASE OPENFIELD

my_comparisons <- list( c("E22", "E33"), (c("E22", "E44")), (c("E33", "E44")) )

#pairwise testing between age
stat.test <- dfBaseOpen %>%
  group_by(APOE) %>%
  t_test(Total.Distance~Age_Cat) %>%
...
gBase<-ggplot(dfBaseOpen, aes(x=APOE, y=Total.Distance, color = Age_Cat)) + geom_violin(position=position_dodge(1)) + geom_boxplot(position=position_dodge(1), width = 0.15) + geom_jitter(position=position_dodge(1)) +  stat_n_text()+ labs(title="Total distance in E22, E33, E44 genotype in openfield", x = "APOE_Genotypes",y ="Total Distance (m)")+ theme(plot.title = element_text(hjust = 0.5), panel.background=element_blank(), axis.line = element_line(colour = "black"))  + stat_compare_means(method ="anova", label.y= 70) + stat_pvalue_manual(stat.test, label = "p", tip.length = 0, hide.ns = TRUE)
  
gBase

```
```{r}
gBase + facet_grid(rows = vars(Sex), margins=TRUE) + stat_compare_means(comparisons = my_comparisons, step.increase = 0.2)

ggsave('BaseOpenfield_total distance_Compare_Age_.pdf', plot=last_plot(), device='pdf',
...
stat.test <- dfBaseOpen %>%
  group_by(Age_Cat) %>%
  t_test(Total.Distance~APOE) %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj")
stat.test

stat.test <- stat.test %>%
  add_xy_position(x= "Age_Cat", dodge = 0.8)  



gBase<-ggplot(dfBaseOpen, aes(x=Age_Cat, y=Total.Distance, color = APOE)) + geom_violin(position=position_dodge(1)) + geom_boxplot(position=position_dodge(1), width = 0.15) + geom_jitter(position=position_dodge(1)) +  stat_n_text()+ labs(title="Total distance in E22, E33, E44 genotype in openfield", x = "APOE_Genotypes",y ="Total Distance (m)")+ theme(plot.title = element_text(hjust = 0.5), panel.background=element_blank(), axis.line = element_line(colour = "black"))  + stat_compare_means(method ="anova", label.y= 47) + stat_compare_means(comparisons = my_comparisons)  #+ stat_pvalue_manual(stat.test, label = "p.adj", tip.length = 0, hide.ns = TRUE)
gBase

 #ggsave('Base3Openfield.pdf', plot=last_plot(), device='pdf', scale=1, width=9, height=7, unit=c("in"), dpi=300)  + stat_pvalue_manual(stat.test, label = "p", tip.length = 0, hide.ns = TRUE)
...
```{r}
gBase +  facet_grid(rows = vars(Sex), margins=TRUE) + stat_compare_means(comparisons = my_comparisons) 

ggsave('Base3Openfield_total distance_Compare_Age_.pdf', plot=last_plot(), device='pdf',
       scale=1, width=9, height=11, unit=c("in"), dpi=300)  + stat_pvalue_manual(stat.test, label = "p", tip.length = 0, hide.ns = TRUE)
...
```{r}
#COMPARING SEX FOR BASE OPENFIELD
...
gBase<-ggplot(dfBaseOpen, aes(x=APOE, y=Total.Distance, color = Sex)) + geom_violin(position=position_dodge(1)) + geom_boxplot(position=position_dodge(1), width = 0.15) + geom_jitter(position=position_dodge(1)) +  stat_n_text()+ labs(title="Total distance in E22, E33, E44 genotype in openfield", x = "APOE_Genotypes",y ="Total Distance (m)")+ theme(plot.title = element_text(hjust = 0.5), panel.background=element_blank(), axis.line = element_line(colour = "black"))  + stat_compare_means(method ="anova", label.y= 70) 
gBase
```

```{r}
gBase + facet_grid(rows = vars(Age_Cat), margins=TRUE) 

ggsave('BaseOpenfield_total distance_Compare_Sex_.pdf', plot=last_plot(), device='pdf',
...
ggsave('YoungViolinsOpenfield`_total distance_HN Genotypes_box Age_Sex_.pdf', plot=last_plot(), device='pdf',
       scale=1, width=9, height=11, unit=c("in"), dpi=300)
